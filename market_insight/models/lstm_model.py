...
class LSTM(nn.Module):
    def __init__(self, input_size=1, hidden_layer_size=100, output_size=2):  # Set output_size to 2
        super().__init__()
        self.logger = setup_console_logger('LSTMLogger')
        self.hidden_layer_size = hidden_layer_size
        self.lstm = nn.LSTM(input_size, hidden_layer_size, batch_first=True)
        self.linear = nn.Linear(hidden_layer_size, output_size)  # Output size is 2
    ...
    def forward(self, input_seq):
        ...
        predictions = self.linear(lstm_out[:, -1, :])  # No need to reshape here
